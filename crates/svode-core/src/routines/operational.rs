...
/// Replace the cached definition rows of one owner with the rows of a freshly
/// read catalog snapshot. Rows without a portable identity stay uncached.
pub async fn replace_catalog_snapshot(
    ...
    let rows = snapshot
        ...
        .map_err(|error| RoutineStoreError::General(error.to_string()))?;
    replace_owner_snapshot(
        pool,
        &snapshot.owner.owner_path,
        snapshot.owner.kind == RoutineOwnerKind::Collection,
...
/// The latest run of one Routine as the shared run projection.
pub async fn latest_run_record(
    pool: &SqlitePool,
    owner_path: &str,
    routine_id: &str,
) -> Result<Option<RoutineRunRecord>, RoutineStoreError> {
    Ok(latest_run(pool, owner_path, routine_id)
        .await?
