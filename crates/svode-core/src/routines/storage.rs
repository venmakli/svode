...
) -> Result<OpenOutcome, RoutineStoreError> {
    if !db_path.exists() {
...
async fn connect(db_path: &Path, create_if_missing: bool) -> Result<SqlitePool, RoutineStoreError> {
...
async fn schema_status(pool: &SqlitePool) -> Result<SchemaStatus, RoutineStoreError> {
...
pub async fn initialize_schema(pool: &SqlitePool) -> Result<(), RoutineStoreError> {
    match schema_status(pool).await? {
        ...
        SchemaStatus::Unsupported(found) => {
            return Err(RoutineStoreError::General(format!(
                "unsupported routines database schema (found {found:?}, expected {SCHEMA_VERSION})"
            )));
        }
    }
...
pub fn database_path(space_dir: &Path) -> PathBuf {
