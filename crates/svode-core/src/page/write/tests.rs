...
async fn field_batch_renames_before_final_relation_projection_and_reports_full_changes() {
    ...
    let outcome = save_fields(
        ...
        "tasks/Old.md",
        BTreeMap::from([
            ("Person".into(), serde_json::Value::String("Ada.md".into())),
            ("note".into(), serde_json::Value::String("{{date}}".into())),
            ("description".into(), serde_json::Value::String("  ".into())),
            ("id".into(), serde_json::Value::String("external-id".into())),
            ("obsolete".into(), serde_json::Value::Null),
            ("unknown".into(), serde_json::Value::String("kept".into())),
            ("title".into(), serde_json::Value::String("New".into())),
        ]),
    )
    .await
    .unwrap();

    assert_eq!(outcome.result.new_path.as_deref(), Some("tasks/New.md"));
...
async fn field_batch_rejects_read_only_fields_before_valid_changes() {
    ...
    for read_only in ["created", "updated", "uid"] {
        let result = save_fields(
            ...
            "tasks/Old.md",
            BTreeMap::from([
                ("note".into(), serde_json::Value::String("after".into())),
                (read_only.into(), serde_json::Value::Null),
            ]),
        )
        .await;
...
async fn field_batch_prevalidates_all_values_before_title_or_custom_write() {
    ...
    let result = save_fields(
        ...
        BTreeMap::from([
            ...
            ("count".into(), serde_json::Value::Number(2.into())),
            ("title".into(), serde_json::Value::String("New".into())),
        ]),
    )
    .await;
