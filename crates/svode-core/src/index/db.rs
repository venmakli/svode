...
pub async fn create_pool(db_path: &Path) -> Result<SqlitePool, IndexError> {
...
pub fn is_corrupt_database_error(error: &impl std::fmt::Display) -> bool {
...
pub fn quarantine_database_family(
    db_path: &Path,
...
fn quarantine_database_family_at(
    ...
    let name = db_path
        ...
        .ok_or_else(|| IndexError::Index("invalid database filename".into()))?;
    ...
    let backups = loop {
        ...
        stamp = stamp
            .checked_add(1)
            .ok_or_else(|| IndexError::Index("quarantine timestamp exhausted".into()))?;
    };
...
pub async fn ensure_schema(pool: &SqlitePool) -> Result<(), IndexError> {
