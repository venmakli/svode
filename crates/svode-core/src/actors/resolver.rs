...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct ActorInvalidationPayload {
    pub repository_id: String,
...
impl ActorSnapshot {
    ...
    pub fn mutation_actor(&self, email: &str) -> Option<MutationActor> {
...
impl ActorCatalogState {
    ...
    pub async fn snapshot(
        ...
        cli: &GitCli,
        space_path: &Path,
    ) -> Result<Arc<ActorSnapshot>, ActorError> {
    ...
    async fn snapshot_at_repository(
        ...
        let unchanged = self
            .source_stamps
            .lock()
            .map_err(|_| ActorError::General("actor source cache poisoned".into()))?
            .get(repository)
            == Some(&stamp);
        if !unchanged {
            self.mark_repository_dirty(repository)?;
    ...
    pub async fn activity(
        ...
        cursor: Option<&str>,
    ) -> Result<ActorActivity, ActorError> {
        let repository = resolve_repository(cli, space_path).await?;
        ...
        let row = snapshot
            ...
            .ok_or_else(|| {
                ActorError::FileNotFound(format!(
                    ...
                    Ok(activity) => {
                            ...
                            .is_some_and(|current| current.generation == snapshot.generation)
                        {
                            self.activities
                                .lock()
                                .map_err(|_| {
                                    ActorError::General("actor activity cache lock poisoned".into())
                                })?
    ...
    fn cached(&self, repository: &Path) -> Result<Option<Arc<ActorSnapshot>>, ActorError> {
        self.snapshots
            .lock()
            .map(|snapshots| snapshots.get(repository).cloned())
            .map_err(|_| ActorError::General("actor snapshot cache lock poisoned".into()))
    }

    fn cached_current(&self, repository: &Path) -> Result<Option<Arc<ActorSnapshot>>, ActorError> {
    ...
    fn repository_revision(&self, repository: &Path) -> Result<RepositoryRevision, ActorError> {
        self.revisions
            .lock()
            .map(|revisions| revisions.get(repository).copied().unwrap_or_default())
            .map_err(|_| ActorError::General("actor repository revision cache poisoned".into()))
    }

    pub fn mark_repository_dirty(
        &self,
        repository: &Path,
    ) -> Result<ActorInvalidationPayload, ActorError> {
        let repository = fs::canonicalize(repository).map_err(|error| {
            ActorError::GitCommandFailed(format!(
    ...
    pub fn repository_lock(&self, repository: &Path) -> Result<Arc<AsyncMutex<()>>, ActorError> {
        let mut locks = self
            .repository_locks
            .lock()
            .map_err(|_| ActorError::General("actor repository lock cache poisoned".into()))?;
        Ok(locks
    ...
    ) -> Result<Option<Arc<ActorActivityYear>>, ActorError> {
        self.activities
            .lock()
            .map(|activities| activities.get(key))
            .map_err(|_| ActorError::General("actor activity cache lock poisoned".into()))
    }

    fn activity_lock(&self, key: &ActivityCacheKey) -> Result<Arc<AsyncMutex<()>>, ActorError> {
        let mut locks = self
            .activity_locks
            .lock()
            .map_err(|_| ActorError::General("actor activity lock cache poisoned".into()))?;
        Ok(locks
            .entry(key.clone())
            .or_insert_with(|| Arc::new(AsyncMutex::new(())))
            .clone())
    ...
    pub async fn load_and_publish(
        ...
        loop {
            ...
            let mut revisions = self.revisions.lock().map_err(|_| {
                ActorError::General("actor repository revision cache poisoned".into())
            })?;
...
fn activity_year_range(year: i32, today: NaiveDate) -> Result<(NaiveDate, NaiveDate), ActorError> {
    if year > today.year() {
        return Err(ActorError::General(
    ...
    let range_start = NaiveDate::from_ymd_opt(year, 1, 1)
        .ok_or_else(|| ActorError::General("actor activity year is out of bounds".into()))?;
    let range_end_exclusive = if year == today.year() {
        today
            .checked_add_days(chrono::Days::new(1))
            .ok_or_else(|| {
                ActorError::General("actor activity current-day boundary is out of bounds".into())
            })?
    } else {
...
async fn load_actor_history(cli: &GitCli, repository: &Path) -> Result<String, ActorError> {
    let history = cli
        .exec_with_env(
            repository,
            &[
    ...
    if history.exit_code != 0 && !is_unborn_repository(cli, repository).await? {
        return Err(ActorError::GitCommandFailed(format!(
...
async fn load_activity_year(
    ...
    range_start: NaiveDate,
    range_end_exclusive: NaiveDate,
) -> Result<ActorActivityYear, ActorError> {
...
fn parse_activity_day(
    ...
    range_start: NaiveDate,
    range_end_exclusive: NaiveDate,
) -> Result<NaiveDate, ActorError> {
    let day = NaiveDate::parse_from_str(value, "%Y-%m-%d")
        .map_err(|_| ActorError::General("actor activity day must use YYYY-MM-DD".into()))?;
...
fn validate_activity_cursor(
    ...
    key: &ActivityCacheKey,
    selected_day: Option<NaiveDate>,
) -> Result<ActivityCursorPosition, ActorError> {
    fn invalid_cursor() -> ActorError {
        ActorError::General("actor activity cursor is stale or does not match this request".into())
    }
...
pub async fn resolve_repository(cli: &GitCli, space_path: &Path) -> Result<PathBuf, ActorError> {
    let output = cli
...
pub async fn load_snapshot(
    ...
    repository: &Path,
    generation: u64,
) -> Result<ActorSnapshot, ActorError> {
...
async fn is_shallow_repository(cli: &GitCli, repository: &Path) -> Result<bool, ActorError> {
    let output = cli
...
async fn is_unborn_repository(cli: &GitCli, repository: &Path) -> Result<bool, ActorError> {
    let output = cli
...
async fn current_git_identity(
    cli: &GitCli,
    repository: &Path,
) -> Result<Option<Identity>, ActorError> {
...
"#,
    ...
    async fn exact_day_uses_full_year_and_has_independent_bounded_continuation() {
        ...
        assert!(matches!(cross_mode, ActorError::General(_)));
        ...
        assert!(matches!(stale, ActorError::General(_)));
    }
    ...
    async fn root_and_inline_reuse_activity_until_snapshot_generation_changes() {
        ...
        assert!(matches!(error, ActorError::FileNotFound(_)));
    }
