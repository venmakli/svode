...
/// Per-workspace SQLite index state managed by Tauri.
///
/// Holds a pool of `SqlitePool` connections keyed by workspace path.
...
pub struct IndexState {
    ...
    /// Per-workspace serialization lock for `full_reindex`. Two rapid
    /// `open_workspace` calls would otherwise spawn two concurrent reindexes
...
impl IndexState {
    ...
    /// Get (or create) the per-workspace reindex serialization lock.
    pub async fn reindex_lock(&self, workspace_path: &str) -> Arc<Mutex<()>> {
    ...
    /// Get an existing pool for the workspace, or open one (creating the DB
    /// file and schema if necessary).
    pub async fn get_or_create(&self, workspace_path: &str) -> Result<SqlitePool, AppError> {
        {
        ...
        if let Some(existing) = pools.get(workspace_path) {
    ...
    /// Drop the pool for a workspace and close its underlying connections.
    ///
    ...
    /// (workspace deletion / app shutdown).
    pub async fn close(&self, workspace_path: &str) {
        let pool = {
            let mut pools = self.pools.lock().await;
            pools.remove(workspace_path)
        };
        if let Some(pool) = pool {
            tracing::info!("closing index pool for workspace {workspace_path}");
            pool.close().await;
        }
    }
}
