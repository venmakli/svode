...
impl PendingPaths {
    pub(crate) fn begin_save(&self, space: &Path, anchors: Option<&[PathBuf]>) -> PendingSave {
        let mut map = self.pending.lock().unwrap();
    ...
    pub(crate) fn record(&self, space: &Path, op: Option<StructuralOp>, paths: Vec<PathBuf>) {
        let mut map = self.pending.lock().unwrap();
        let entry = map
