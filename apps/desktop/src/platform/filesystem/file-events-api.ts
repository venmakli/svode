...
export function listenFileCreated(
  ...
  return listen<FileEventDto>("file:created", (event) => handler(event.payload));
}

export function listenFileChanged(
  handler: FileEventHandler,
): Promise<UnlistenFn> {
  return listen<FileEventDto>("file:changed", (event) => handler(event.payload));
}

export function listenFileDeleted(
  handler: FileEventHandler,
): Promise<UnlistenFn> {
  return listen<FileEventDto>("file:deleted", (event) => handler(event.payload));
}
