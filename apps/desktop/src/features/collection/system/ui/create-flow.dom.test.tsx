...
import type {
  SystemCollectionCreateFlowFocusRequest,
  SystemCollectionCreateFlowProps,
} from "./create-flow";

type CreateFlowComponent =
  typeof import("./create-flow").SystemCollectionCreateFlow;

const isolatedDomProcess =
  process.env.SVODE_SYSTEM_COLLECTION_CREATE_FLOW_DOM_PROCESS === "1";

if (!isolatedDomProcess) {
  test("System Collection create-flow DOM scenarios", () => {
    const child = spawnSync(
      ...
      ["test", fileURLToPath(import.meta.url)],
      {
        env: {
          ...process.env,
          SVODE_SYSTEM_COLLECTION_CREATE_FLOW_DOM_PROCESS: "1",
        },
        encoding: "utf8",
      },
    );
  ...
  test("create flow owns progress, focus, scroll, discard, and return focus", async () => {
    ...
    const { SystemCollectionCreateFlow } = await import("./create-flow");

    try {
      await act(async () => {
        root.render(<Harness Flow={SystemCollectionCreateFlow} />);
        await nextTurn();
      });
...
function Harness({ Flow }: { Flow: CreateFlowComponent }) {
  ...
  const moveTo = (
    next: number,
    target: SystemCollectionCreateFlowFocusRequest["target"],
  ) => {
...
function currentStep(dom: JSDOM) {
  return dom.window.document
    .querySelector('[data-system-collection-create-flow="test-flow"]')
