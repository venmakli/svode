...
test("structured Collection rows activate from the row and keyboard, preserve nested controls, and expose the context menu", async () => {
  const dom = new JSDOM(
  ...
  try {
    ...
    const headerActions = renderToStaticMarkup(<>{activations[0]?.actions}</>);
...
test("neutral Table preserves row focus, activation, and nested action boundaries", async () => {
  const dom = new JSDOM(
    "<!doctype html><html><body><div id=app></div></body></html>",
  ...
  const presentation = defineCollectionPresentation<Row>({
      ...
      properties: [
        {
          ...
          owner: { featureId: "attachments", kind: "feature" },
    ...
    state: {
      phase: "ready",
      rows: [
        { id: "one", name: "brief.pdf" },
        { id: "two", name: "notes.md" },
      ],
    },
  });
  const root = createRoot(dom.window.document.getElementById("app")!);
  ...
  try {
    ...
    const first = dom.window.document.querySelector<HTMLElement>(
      '[data-collection-row="one"]',
    )!;
