...
import { CollectionQueryToolbarButton } from "../../ui/presentation-chrome";
import {
  createDefaultCollectionFilterRule,
...
import {
  ...
  QueryAddButton,
  QueryEditorFooter,
  CollectionPropertyChoiceList,
} from "./query-editor-parts";

type CollectionQueryPane =
  | "filter"
  | "filterField"
  | "filterEditor"
  | "sort"
...
export function CollectionQueryEditor({
  ...
  function setEditorPane(nextPane: CollectionQueryPane) {
  ...
  const resetWarningNotice = resetWarning ? (
    <Alert>
      <AlertTriangle />
      <AlertTitle>{m.collection_query_reset_title()}</AlertTitle>
      <AlertDescription>
        {m.collection_query_reset_description()}
      </AlertDescription>
      {onDismissResetWarning ? (
        <AlertAction>
          <Button
          ...
          >
            {m.collection_query_reset_dismiss()}
          </Button>
        </AlertAction>
      ) : null}
    </Alert>
            ...
            <div className="flex items-center gap-1">
              ...
              sortFields.length === 0 ? (
                <CollectionQueryToolbarButton
                  ...
                  aria-haspopup="dialog"
                  icon={AlertTriangle}
                  label={m.collection_query_reset_title()}
                  onClick={() => {
