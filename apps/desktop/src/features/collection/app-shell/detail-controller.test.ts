...
const firstSelection: CollectionDetailSelection = {
  instanceKey: "space:root:actors",
  presentationId: "contributors",
  rowId: "person:one",
};

const secondSelection: CollectionDetailSelection = {
  ...firstSelection,
  rowId: "person:two",
};
...
function request(
  ...
  input: Partial<CollectionDetailRequest> = {},
): CollectionDetailRequest {
  return {
...
test("focus restoration falls back when the original trigger disappeared", () => {
  ...
  const focused = focusCollectionDetailTarget({
    fallbackFocus: () =>
      ({
        focus: () => {
