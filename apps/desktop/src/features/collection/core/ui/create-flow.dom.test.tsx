...
const isolatedDomProcess =
  process.env.SVODE_COLLECTION_CORE_CREATE_FLOW_DOM_PROCESS === "1";

if (!isolatedDomProcess) {
  test("Collection Core create-flow DOM scenarios", () => {
    const child = spawnSync(
      ...
      ["test", fileURLToPath(import.meta.url)],
      {
        env: {
          ...process.env,
          SVODE_COLLECTION_CORE_CREATE_FLOW_DOM_PROCESS: "1",
        },
        encoding: "utf8",
      },
    );
...
function currentStep(dom: JSDOM) {
  return dom.window.document
    .querySelector('[data-collection-core-create-flow="test-flow"]')
