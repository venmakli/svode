...
interface CollectionCorePropertyProps {
  property: CollectionPropertyDefinition<unknown>;
  row: unknown;
}

export function CollectionCoreInlineDiagnostic({
  message,
}: {
  message: string;
}) {
  return (
    <span
      ...
      data-collection-core-diagnostic
    >
      {message}
    </span>
  );
...
export function CollectionCorePropertyValue({
  ...
  return (
    <CollectionCoreInlineDiagnostic
      message={m.collection_core_property_renderer_unavailable({
...
function readPropertyApplicability(
  ...
  try {
    ...
    return {
      label: m.collection_core_callback_error(),
      status: "unavailable",
    };
  }
}

function CollectionCorePropertyPassiveState({
  applicability,
  propertyKey,
}: {
  ...
  return (
    <span
      ...
      data-collection-core-property={propertyKey}
      data-collection-core-property-applicability={applicability.status}
    >
      {applicability.label}
    </span>
  );
...
export function CollectionCorePropertyControl({
  property,
  row,
  density = "default",
  onRejected,
  ...
  if (!column || !edit) {
    return (
      <CollectionCoreInlineDiagnostic
        message={m.collection_core_property_control_unavailable({
          property: property.key,
        })}
      />
    );
  ...
  return (
    <div
      ...
      data-collection-core-property={property.key}
      data-collection-core-property-state={status}
      title={message}
