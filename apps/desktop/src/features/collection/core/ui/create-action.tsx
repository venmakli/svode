...
function readActionState(
  action: CollectionCoreCreateAction,
): CollectionCoreActionState {
  try {
...
function useCollectionCorePresentationAction(
  action: CollectionCoreCreateAction,
...
export function CollectionCoreCreateActionButton({
  action,
  onRejected,
}: {
  action: CollectionCoreCreateAction;
  ...
  const { run, state } = useCollectionCorePresentationAction(
    action,
    onRejected,
  );
  ...
  const button = (
    <Button
      ...
      data-collection-core-create={action.id}
      data-collection-core-create-state={state.status}
      title={state.message}
      onClick={() => void run()}
    >
      {state.pending ? (
