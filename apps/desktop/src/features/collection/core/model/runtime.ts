...
import { collectionCorePresentationRuntimeBrand } from "./types";

const stableIdPattern = /^[A-Za-z0-9](?:[A-Za-z0-9._:-]*[A-Za-z0-9])?$/;

interface ErasedCollectionCorePresentationRuntime extends CollectionCorePresentationRuntime {
  readonly instance: CollectionCorePresentationInstance<unknown>;
  readonly diagnostics: readonly CollectionCoreDeveloperDiagnostic[];
}
...
function validateProperty<Row>(
  property: CollectionCorePresentationInstance<Row>["descriptor"]["properties"][number],
): CollectionCoreDeveloperDiagnostic[] {
  ...
  if (origin !== "schema_backed" && owner.kind !== "feature") {
    diagnostics.push(
      diagnostic(
        "invalid-property-owner",
        `Property "${property.key}" with origin "${origin}" must be owned by a feature adapter.`,
      ),
    );
  }
...
function validateDescriptor<Row>(
  instance: CollectionCorePresentationInstance<Row>,
): CollectionCoreDeveloperDiagnostic[] {
...
function erasePresentation<Row>(
  instance: CollectionCorePresentationInstance<Row>,
): CollectionCorePresentationInstance<unknown> {
  return instance as unknown as CollectionCorePresentationInstance<unknown>;
}

export function defineCollectionCorePresentation<Row>(
  instance: CollectionCorePresentationInstance<Row>,
): CollectionCorePresentationRuntime {
  const diagnostics = [
  ...
  return Object.freeze({
    [collectionCorePresentationRuntimeBrand]: true as const,
    diagnostics: Object.freeze(diagnostics),
    instance: erasePresentation(effectiveInstance),
  }) satisfies ErasedCollectionCorePresentationRuntime;
}

export function readCollectionCorePresentationRuntime(
  runtime: CollectionCorePresentationRuntime,
): Readonly<ErasedCollectionCorePresentationRuntime> {
  return runtime as ErasedCollectionCorePresentationRuntime;
}
