...
test("Actors has no access chrome and open or refresh never probes", async () => {
  const dom = createDom();
  ...
  try {
    ...
    const refresh = dom.window.document.querySelector<HTMLButtonElement>(
      '[data-system-collection-refresh="refresh-actors"]',
    )!;
    await act(async () => {
      refresh.click();
      await nextTurn();
