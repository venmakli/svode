...
import { useApplyPageLinkTarget } from "../hooks/use-page-link-insertion";
import { usePageLinkTargetSearch } from "../hooks/use-page-link-target-search";
import { useEditorDocumentContext } from "../hooks/use-resolved-asset-url";
import { findSpaceById } from "../lib/page-link-utils";

export function PageLinkTargetPicker() {
  ...
  const { items, loading } = usePageLinkTargetSearch({
    localCurrentSpace,
    projectPath,
    query,
    sourceSpaceId,
  });
  const applyPageLinkTarget = useApplyPageLinkTarget(editor);
        ...
        <CommandGroup>
          {items.map((item) => (
            <CommandItem
              ...
              onSelect={() => void applyPageLinkTarget(item)}
