...
import { useInsertPageLinkTarget } from "../hooks/use-page-link-insertion";
...
export function PageLinkInputElement(
  ...
  const { items } = usePageLinkTargetSearch({
    debounceMs: 0,
    localCurrentSpace,
    projectPath,
    query: "",
  ...
  const insertPageLinkTarget = useInsertPageLinkTarget(editor);
          ...
          <InlineComboboxGroup>
            {items.map((item) => (
              <InlineComboboxItem
                ...
                onClick={() => void insertPageLinkTarget(item)}
              >
